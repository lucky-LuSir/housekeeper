<template>
    <div class="pre-scrollable-s">
        ...
        <cus-entrust-condition ref="cusFollowupCondition" v-if="cusFollowupConditionVisible" @changeCus="serchCusClick"/>
        <!--客户详情面板-->
            ...
            <el-table
                ...
                <el-table-column
                    ...
                    <template slot-scope="scope">
                        <span v-if="scope.row.status === 1"><el-tag type="info"   size="mini">待分配</el-tag></span>
                        <span v-if="scope.row.status === 2"><el-tag type="warning" size="mini">待处理</el-tag></span>
                        <span v-if="scope.row.status === 3"><el-tag type="success" size="mini">已转化</el-tag></span>
                        <span v-if="scope.row.status === 4"><el-tag type="error"   size="mini">作废</el-tag></span>
                    </template>
                </el-table-column>
...
<script>
    import cusEntrustCondition from "./cusEntrustCondition.vue"
...
//                alert("开始showIfEntrustBtn");
            ...
            addCus(index,rows){
                ...
                this.$nextTick(() => {
                    ...
                    this.$refs.refCusAdd.cusForm.cusName=rows.linkman;
                    this.$refs.refCusAdd.cusForm.cusPhone=rows.linkmanPhone;
                    this.$refs.refCusAdd.cusForm.cusType=1;
                    this.$refs.refCusAdd.cusForm.cusFrom='官网预约';

                    this.$refs.refCusAdd.cusEntrustSendObj.sendName="EntrustSend";
                    this.$refs.refCusAdd.cusEntrustSendObj.entrustId=rows.id;
