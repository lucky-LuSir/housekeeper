<template>
    <el-dialog :style="{height: fullHeight +80 +'px'}" top="50px" :before-close="closeDialog" append-to-body
        ...
        <div>
            <!--客户详情面板-->
            <cus-details ref="cusDetailsx" @refresh="getFocusfocusCusRecord" v-if="cusDetailsOpenOrClose"/>
            <div style="font-weight: bold;text-align: center;margin-bottom:20px">
                <p style="font-size: 20px;">集中获客客户推送记录</p>
            </div>
            <div style="margin-bottom: 20px">
                <el-input style="width: 300px;"
                          size="small"
                          placeholder="请输入客户号码"
                          v-model="focusQuery.cusPhone"
                          clearable>
                    <el-button slot="append" icon="el-icon-search" @click="getFocusfocusCusRecord()"/>
                </el-input>
            </div>
            <!--数据表格-->
            <el-table
                ...
                <el-table-column type="expand" prop="userList">
                    <template slot-scope="scope">
                        <el-table border :data="scope.row.userList" label-position="left">
                            <el-table-column prop="userCode" label="用户编码" align="center" width="200px">
                                <template slot-scope="scope">
                            ...
                            <el-table-column prop="ownerDeptName" label="所属部门" align="center" width="150px">
                                <template slot-scope="scope">
                                    <span>{{scope.row.ownerDeptName}}</span>
                                </template>
                            </el-table-column>
                            <el-table-column prop="focusTime" label="收到客户推送提醒时间" align="center" width="200px">
                                <template slot-scope="scope">
                                    <span>{{scope.row.focusTime | time}}</span>
                                </template>
                            </el-table-column>
                            <el-table-column prop="seeCusTime" label="客户号码可查看时间" align="center" width="200px">
                                <template slot-scope="scope">
                                    <span>{{scope.row.seeCusTime | time}}</span>
                                </template>
                            </el-table-column>
                            <el-table-column prop="followCount" label="跟进当前客户次数" align="center" width="150px">
                                <template slot-scope="scope">
                                    <span>{{scope.row.followCount}}</span>
                                </template>
                            </el-table-column>
    ...
    export default {
            ...
            selectData(data) {
                ...
                if (data.length > 0) {
                    data.forEach((item, index) => {
                        this.selectRow.push(this.focusCusRecord.indexOf(item));
                    });
                }
            },
            fullHeight(val) {
        ...
        methods: {
            ...
            handleCurrentChange(val) {
                ...
                this.getFocusfocusCusRecord();
            },
            //查询客户
            getFocusfocusCusRecord() {
                var vm = this;
                vm.loading = true;
                var sendObj = {};
                ...
                var option = {
                    ...
                    data: sendObj,
                    url: "customer/focusCusQuery",
                };
                this.$ajax(
                    option
                ).then(function (response) {
                    vm.focusCusRecord = response.data.result.data;
