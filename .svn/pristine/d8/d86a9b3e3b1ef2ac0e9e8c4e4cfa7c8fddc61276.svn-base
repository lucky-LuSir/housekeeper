<template>
    <el-dialog :style="{height: fullHeight +80 +'px'}" top="50px" :before-close="closeDialog" append-to-body
        ...
        <div>
            <!--客户详情面板-->
            <cus-details ref="cusDetailsx" @refresh="getFocusfocusCusList" v-if="cusDetailsOpenOrClose"/>
            <div style="font-weight: bold;text-align: center;margin-bottom:20px">
                <p style="font-size: 20px;">优先推送客户</p>
            </div>
            <div style="font-weight: bold;text-align: center;margin-bottom:10px">
                <el-button type="primary" round size="mini" style="font-size: 13px;">
                    以下是公司获客，优先推送给您，客户只有包含您在内的3个人可见！请珍惜客户资源，尽快跟进及带看！
                </el-button>
            </div>
            ...
            <el-table
                ...
                <el-table-column
                        ...
                        align="center"
                        width="140">
                    <template slot-scope="scope" v-if="scope.row.closeTime!=null">{{ scope.row.closeTime}} 小时
                    </template>
                </el-table-column>
                <el-table-column
                        prop="cusName"
    ...
    export default {
            ...
            selectData(data) {
                ...
                if (data.length > 0) {
                    data.forEach((item, index) => {
                        this.selectRow.push(this.focusCusList.indexOf(item));
                    });
                }
            },
            fullHeight(val) {
        ...
        methods: {
            ...
            handleCurrentChange(val) {
                ...
                this.getFocusfocusCusList();
            },
            //查询客户
            getFocusfocusCusList() {
                var vm = this;
                vm.loading = true;
                var sendObj = {};
                ...
                var option = {
                    ...
                    data: sendObj,
                    url: "customer/queryFocusCus",
                };
                this.$ajax(
                    option
                ).then(function (response) {
                    vm.focusCusList = response.data.result.data;
